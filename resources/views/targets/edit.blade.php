<x-app-layout>
    <x-slot name="header_title">Edit Target: {{ Str::limit($target->name ?: $target->url, 30) }}</x-slot>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-2xl text-slate-800 dark:text-slate-100 leading-tight">
                {{ __('Edit Target') }}
            </h2>
            <a href="{{ route('targets.history', $target) }}" class="text-sm font-medium text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300 transition-colors">
                View History &rarr;
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-8 md:py-12">
        <div class="max-w-xl mx-auto px-2 sm:px-4 md:px-6">
            <div class="bg-white dark:bg-slate-800 shadow-xl rounded-2xl p-6 md:p-8">
                <form method="POST" action="{{ route('targets.update', $target) }}" class="space-y-6">
                    @csrf
                    @method('PATCH')
                    <div>
                        ...
                        <x-text-input id="url" name="url" type="text" class="block w-full mt-1" 
                                      placeholder="e.g. www.google.com" value="{{ old('url', $target->url) }}" 
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="block w-full mt-1" 
                                      placeholder="e.g. My Company Website" value="{{ old('name', $target->name) }}" 
                                      autocomplete="off" />
